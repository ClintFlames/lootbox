...
public class LootboxGuiT1ButtonMessage extends FriendlyByteBuf {
	public LootboxGuiT1ButtonMessage(int buttonID, int x, int y, int z) {
	...
	public static void apply(MinecraftServer server, ServerPlayer entity, ServerGamePacketListenerImpl handler, FriendlyByteBuf buf, PacketSender responseSender) {
		...
		server.execute(() -> {
			...
			HashMap guistate = LootboxGuiT1Menu.guistate;
			if (buttonID == 0) {
