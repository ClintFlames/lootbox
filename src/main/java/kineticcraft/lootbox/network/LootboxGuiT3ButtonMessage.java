...
public class LootboxGuiT3ButtonMessage extends FriendlyByteBuf {
	public LootboxGuiT3ButtonMessage(int buttonID, int x, int y, int z) {
	...
	public static void apply(MinecraftServer server, ServerPlayer entity, ServerGamePacketListenerImpl handler, FriendlyByteBuf buf, PacketSender responseSender) {
		...
		server.execute(() -> {
			...
			HashMap guistate = LootboxGuiT3Menu.guistate;
			if (buttonID == 0) {
