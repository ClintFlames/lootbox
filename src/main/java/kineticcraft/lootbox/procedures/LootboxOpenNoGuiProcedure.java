...
public class LootboxOpenNoGuiProcedure {

	public static InteractionResult execute(Map<String, Object> dependencies) {
		if (dependencies.get("entity") == null) {
			if (!dependencies.containsKey("entity"))
				LootboxMod.LOGGER.warn("Failed to load dependency entity for procedure LootboxOpenNoGui!");
		...
		if (entity instanceof Player _playerHasItem ? _playerHasItem.getInventory().contains(t1) : false) {
			random = Math.floor(Mth.nextDouble(RandomSource.create(), 0, 11));
			if (entity instanceof Player _player) {
				ItemStack _stktoremove = t1;
				_player.getInventory().clearOrCountMatchingItems(p -> _stktoremove.getItem() == p.getItem(), 1, _player.inventoryMenu.getCraftSlots());
			}
		} else if (entity instanceof Player _playerHasItem ? _playerHasItem.getInventory().contains(t2) : false) {
			random = Math.floor(Mth.nextDouble(RandomSource.create(), 0, 17));
			if (entity instanceof Player _player) {
				ItemStack _stktoremove = t2;
				_player.getInventory().clearOrCountMatchingItems(p -> _stktoremove.getItem() == p.getItem(), 1, _player.inventoryMenu.getCraftSlots());
			}
		} else if (entity instanceof Player _playerHasItem ? _playerHasItem.getInventory().contains(t3) : false) {
